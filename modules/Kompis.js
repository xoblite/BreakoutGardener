...
function Log()
{
	console.log("Breakout Gardener -> KOMPIS -> Hello again... =]");
}
...
function Display(refreshAll) // Show our little animated friend "Kompis"! :)
{
	...
	if (IS31FL3731.IsAvailable())
	{
        ...
        // (Note: Since the animation frame is increased after each
        // "SH1107" loop above we need to use the "blink" frames+1 here...)
        if (animationFrame == 11 || animationFrame == 21 || animationFrame == 40) IS31FL3731.Display(heart);
        else
        {
